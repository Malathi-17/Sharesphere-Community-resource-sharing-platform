...
app.post("/resource", upload.single("image"), async (req, res) => {
  const { community_id, user_id, name, description, category, quantity, item_condition } = req.body;
  ...
  try {
    ...
    const approval_status = admin[0]?.role === "ADMIN" ? "APPROVED" : "PENDING";
    await query(
      `INSERT INTO resources (community_id, owner_id, name, description, category, quantity, available_quantity, item_condition, image_url, approval_status)
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [community_id, user_id, name, description, category, quantity || 1, quantity || 1, item_condition || "Good", image_url, approval_status]
    );
